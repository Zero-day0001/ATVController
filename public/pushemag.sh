...
rm outputs/emagisk-push.log
exec > outputs/emagisk-push.log 2>&1
echo Setting Job
job=2
...
for i in `cat scripts/ips` ; do
  if [[ $i =~ "{".* ]] ; then
    ...
    for (( j = $first ; j <= $last ; j++ )) ; do
      ...
      echo Pushing eMagisk .zip and .config to $ip
      sleep 1
      adb push apps/emagisk.config /data/local/tmp > /dev/null 2>&1
      adb push apps/eMagisk.zip /sdcard > /dev/null 2>&1
      adb shell "su -c 'magisk --install-module /sdcard/eMagisk.zip && reboot'"
      adb kill-server
    done
  else
    ip="$lanip.$i"
